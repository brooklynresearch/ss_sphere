...
public final class SceneRenderer {
     ...
     */
  /* package */ SceneRenderer(Handler uiHandler,
    ...
    public static SceneRenderer createFor2D() {
        return new SceneRenderer(null, null);
    }
     ...
     * initializing the object on the GL thread.
     *
     * <p>The also creates a {@link } that is bound to the VR scene. The View is backed by
     * a {@link } and is meant to be rendered in a VR scene.
     *
     * @param context the {@link Context} used to initialize the {@link }
     * @param parent the new view is attached to the parent in order to properly handle Android
     *     events
     * @return a SceneRender configured for VR and a bound {@link } that can be treated
    ...
    /**
     * Processes Daydream Controller clicks and dispatches the event to {@link } as a
