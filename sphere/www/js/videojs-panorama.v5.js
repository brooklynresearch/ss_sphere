...
var _urSymbol = Symbol();
var _urSymbolevent = Symbol();
var _urSymbolplay = Symbol('nativeplay');
var _urSymbolpause = Symbol('nativepause');
...
function update(timeDiff) {
	...
	if (player.video.ended) {
		delete player.video[_urSymbolevent]; // allow timeupdate event
...
function overloadAPI(video) {
	var player = video[_urSymbol];
	video[_urSymbolplay] = video.play;
	video[_urSymbolpause] = video.pause;
	video.play = play;
	video.pause = pause;
	...
	preventEvent(video, 'timeupdate', _urSymbolevent, false);
	preventEvent(video, 'ended', _urSymbolevent, false); // prevent occasional native ended events
}
...
function enableInlineVideo(video, hasAudio, onlyWhitelisted) {
	...
	if ((onlyWhitelisted && !isWhitelisted) || video[_urSymbol]) {
		return;
	}
